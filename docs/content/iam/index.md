...
The IAM module creates a role for the EC2 instance to assume, with a scoped policy allowing it to
read from the S3 bucket. The role is wrapped in an instance profile so EC2 can attach it.
    ...
    Statement = [
      {
        ...
        # ListBucket is a bucket-level action; GetObject is object-level — each needs its own ARN pattern.
        Resource = [
...
# EC2 doesn't attach roles directly — it attaches an instance profile that wraps the role.
...
The role and the policy are independent resources connected by the `aws_iam_role_policy_attachment`.
This separation is what makes managed policies reusable — the same policy could be attached to
multiple roles without duplication.
...
Terraform automatically applies `module.iam` before `module.ec2` because it sees the reference —
...
If both commands succeed, the instance profile is working. Try a bucket you don't own to confirm
the policy is scoped — it should return `Access Denied`.
